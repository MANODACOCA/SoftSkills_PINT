...
DROP TABLE IF EXISTS ENTREGA_TRABALHOS;
...
create table UTILIZADOR (
   ...
   DATA_CRIACAO_UTILIZ  timestamp with time zone   not null DEFAULT NOW(),
   TELEMOVEL            int                  	null,
   GENERO               int                  	null,
   MORADA               varchar(100)        	null,
   PAIS                 varchar(100)         	null,
   DATA_NASC            date		            null,
   DATA_ATIV_UTILI      timestamp with time zone     null,
   AUTEN2FAT            boolean                  null,
   ESTADO_UTILIZADOR    boolean                 not null DEFAULT TRUE,
...
create table TWOFA (
   ...
   DATA_FA              timestamp with time zone     not null DEFAULT NOW(),
...
create table CURSOS (
   ...
   DATA_INICIO_CURSO    timestamp with time zone             not null,
   DATA_FIM_CURSO       timestamp with time zone             not null,
...
create table AULAS (
   ...
   TEMPO_DURACAO        interval check (TEMPO_DURACAO >= interval '0'),
...
create table CONTEUDOS_PARTILHADO (
   ...
   ID_TOPICO            int                  not null,
   DATA_CRIACAO_CP      timestamp with time zone             not null DEFAULT NOW(),
...
create table NOTIFICACOES_COMENTARIOS_POST (
   ...
   ID_UTILIZADOR        int                  not null,
   DATA_HORA_NOTIFICACAOCP timestamp with time zone          not null DEFAULT NOW(),
...
create table MATERIAL_APOIO (
   ...
   NOME_MATERIAL	    varchar(300)         not null,       
...
create table TRABALHOS (
	ID_TRABALHO		int generated always as identity(start with 1 increment by 1)    not null,
	ID_CURSO_TR 	int 			not null,
	ID_FORMATO_TR	int 			not null,
	NOME_TR			varchar(300)	not null,
	CAMINHO_TR		varchar(1024)	not null,
	DESCRICAO_TR 	varchar(1024)   not null,
	constraint PK_TRABALHOS primary key (ID_TRABALHO),
	constraint FK_TRABALHOS_TEM_CURSO foreign key (ID_CURSO_TR)
		references CURSOS (ID_CURSO),
	constraint FK_TRABALHOS_TEM_FORMATO foreign key (ID_FORMATO_TR)
		references TIPO_FORMATO (ID_FORMATO)
);

create table ENTREGA_TRABALHOS (
	ID_ENTREGA_TRABALHO int generated always as identity(start with 1 increment by 1)    not null,
	ID_TRABALHO_ET		int 		not null,
	ID_FORMANDO_ET	int  			null,
	ID_FORMATO_ET	int 			not null,
	NOME_ET			varchar(300)	not null,
	CAMINHO_ET		varchar(1024)	not null,
	constraint PK_ENTREGA_TRABALHOS primary key (ID_ENTREGA_TRABALHO),
	constraint FK_ENTREGA_TRABALHOS_TEM_FORMANDO foreign key (ID_FORMANDO_ET)
		references FORMANDOS (ID_FORMANDO),
	constraint FK_ENTREGA_TRABALHOS_TEM_TRABALHO foreign key (ID_TRABALHO_ET)
		references TRABALHOS (ID_TRABALHO),
	constraint FK_ENTREGA_TRABALHOS_TEM_FORMATO foreign key (ID_FORMATO_ET)
		references TIPO_FORMATO (ID_FORMATO)
);
...
create table NOTIFICACOES_CURSO (
   ...
   ID_UTILIZADOR        int                  not null,
   ID_CURSO             int                  not null,
   DATA_HORA_NOTIFICACAOCURSO timestamp with time zone       not null DEFAULT NOW(),
   CONTEUDO_NOTIF_CURSO		varchar(500)     not null,
...
create table OCORRENCIAS_EDICOES (
   ...
   DATA_ULT_OCORRENCIA  timestamp with time zone             NOT NULL,
