...
export const update_trabalhos = async (id,
    { id_curso_tr, id_formato_tr, nome_tr, descricao_tr, data_entrega_tr, ficheiro, caminho_tr }
) => {
    try {
        let payload = null;
        let headers = {};
        ...
        if (ficheiro) {
            ...
            if (ficheiro !== undefined) fd.append('ficheiro', ficheiro);
       
            payload = fd;
            headers = { maxBodyLength: ONE_HUNDRED_MB };
        } else {
            payload = { id_curso_tr, id_formato_tr, descricao_tr, data_entrega_tr, nome_tr, caminho_tr };
        }
