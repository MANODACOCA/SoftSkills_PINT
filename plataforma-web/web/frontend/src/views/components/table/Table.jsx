...
const Table = ({ columns, data, actions, onAddClick, conteudos, pesquisa, ordenar, searchTerm = '', onSearchChange, loading }) => {
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPag, setitemsPag] = useState(10);
  const [expandedRows, setExpandedRows] = useState([]);
  const totalPages = Math.max(1,Math.ceil(data.length / itemsPag));
            ...
            {loading ? (
              <tr>
                <td colSpan={columns.length + (actions ? 1 : 0) + (conteudos ? 1 : 0)} className="text-center py-4">
                  <span className="text-muted">A carregar...</span>
                </td>
              </tr>
