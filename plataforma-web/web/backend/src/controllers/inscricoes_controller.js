...
const controllers = {};
const incricaoService = require('../services/incricao.service');
...
controllers.verificarInscricao = async (req,res) => {
  try{
    ...
    const resultado = await incricaoService.checkInscricao(userId, cursoId);
    res.status(200).json(resultado);
  }catch(error){
    console.error('Erro ao verificar inscrição:', error);
    res.status(500).json({erro: 'Erro ao verificar inscrição',desc: error.message});
  }
};
