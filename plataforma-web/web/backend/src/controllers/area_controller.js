...
controllers.create = async (req,res)=>{
  try{
    if(req.body){
      ...
      res.status(400).json({erro: 'Erro ao criar Area!',desc: 'Corpo do pedido esta vazio.'});
    }
  }catch(err){
  ...
  try {
    if(req.body){
      ...
      if(updated){
        ...
        res.status(404).json({erro:'Area nao foi atualizado/a!'});
      }
    }else{
      res.status(400).json({erro: 'Erro ao atualizar o/a Area!',desc: 'Corpo do pedido esta vazio.'});
    }
  }catch(err){
